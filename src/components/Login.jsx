...
const Login = () => {
  ...
  provider: 'github',
  options: {
    redirectTo: 'https://eden-acad.netlify.app/dashboard', // or any route after login
  },
});
  if (error) {
    alert(error.message);
      ...
      <div className="flex items-center justify-center min-h-[calc(100vh-4rem)] px-4 sm:px-6 lg:px-8 py-8">
        <div className="w-full max-w-md">
          ...
          <div className="bg-slate-800/60 backdrop-blur-lg rounded-2xl shadow-2xl border border-slate-700/50 p-6 sm:p-8">
            <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4 space-y-6">
              {/* Email Field */}
              ...
              {/* Login Button */}
              <button className="w-full bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white font-semibold py-3 rounded-lg transition-all duration-200 transform hover:scale-[1.02] focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-slate-800">
                Log In
              </button>
              
              {/* Sign Up Link */}
              <p className="text-center text-slate-400">
                Don't have an account? <NavLink to="/signup" className="text-blue-400 hover:text-blue-300 font-medium transition-colors duration-200">Sign up</NavLink>
              </p>
            </form>
          ...
          <div className="mt-8 text-center text-slate-400 text-sm">
            <p>Secure login powered by EDEN ACADS</p>
          </div>
        </div>
      </div>
    </div>
